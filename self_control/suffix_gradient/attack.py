...
def main():
        ...
        for sentence in tqdm(lines):
            try:
                if "falcon" in args.model or "mpt" in args.model:
                    ...
                    ground_truth_generation = wrapped_model.generate(sentence)
    ...
    if args.use_default:
        logging.info(f"Running default, top_p=0.7, temp=0.95, top_k=500")
        prompts = []
        outputs = []
        model.eval()
            ...
            for sentence in tqdm(lines):
                try:
                    if "falcon" in args.model or "mpt" in args.model:
                        ...
                        ground_truth_generation = wrapped_model.generate(sentence, temperature=temp)
                        # get ground truth generation
                    #     ground_truth_embeds = get_sentence_embedding(
                    #         model, tokenizer, sentence
                    #     )
            ...
            for sentence in tqdm(lines):
                try:
                    ...
                    if "falcon" in args.model or "mpt" in args.model:
                        ...
                        ground_truth_generation = wrapped_model.generate(sentence, top_p=top_p)
                    #     ground_truth_embeds = get_sentence_embedding(
                    #         model, tokenizer, sentence
                    #     )
            ...
            for sentence in tqdm(lines):
                try:
                    ...
                    if "falcon" in args.model or "mpt" in args.model:
                        ...
                        ground_truth_generation = wrapped_model.generate(sentence, top_k=top_k)
                    #     ground_truth_embeds = get_sentence_embedding(
                    #         model, tokenizer, sentence
                    #     )
