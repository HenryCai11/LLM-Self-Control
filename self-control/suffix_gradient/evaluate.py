...
def run(
    ...
    if "default" in config_set:
        ...
        res = pd.read_csv(f"./final_iter.csv")
    ...
    if len(dfs) > 0:
        ...
        concatenated_df.to_csv(f"merged_eval_results/{args.config}/test.csv")
    if len(dfs_text) > 0:
        ...
        concatenated_df_text.to_csv(
            f"merged_eval_results/{args.config}/test_text.csv"
        )
    ...
    json.dump(
        summary,
        open(f"merged_eval_results/{args.config}/test_summary.json", "w"),
        indent=4,
    )
